...
from FDS.util import calibrate_mean_var
...
class FDS(nn.Module):
    ...
    def _get_bucket_idx(self, label):
        ...
        _, bins_edges = np.histogram(a=np.array([], dtype=np.float32), bins=self.bucket_num, range=(-7, 7))
        if label > 7:
    ...
    def update_running_stats(self, features, labels, epoch):
        if epoch < self.epoch:
            print("[WARNING] Epoch [{}] is smaller than the last updated epoch [{}]!".format(epoch, self.epoch))
            return
        ...
        for bucket in np.unique(buckets):
            curr_feats = features[torch.tensor((buckets == bucket), dtype=torch.bool)]
        ...
        print("Updated running statistics with Epoch features!")
